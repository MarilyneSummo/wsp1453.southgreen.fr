...
const toolkitWorkingPath =
	"/opt/www/synflow.southgreen.fr/prod/tmp/toolkit_run/";
...
function sendMail(email, subject, message) {
	mailTransporter.sendMail(
		...
		function (err, info) {
			if (err) {
				logToFile(`Erreur envoi mail: ${err.message}`);
				console.error(err);
			} else {
				logToFile(`Mail envoyé: ${info.response}`);
...
function validateFileContent(filePath, originalName) {
	...
	const dangerousPatterns =
		/<script|javascript:|eval\s*\(|import\s+|require\s*\(|exec\s*\(/i;
	...
	switch (ext) {
		...
		case ".gff":
		case ".gff3":
			if (!/^(##gff-version|#)/.test(trimmed) && !/^\S+\t\S+\t/.test(trimmed))
				return false;
			break;
		case ".bed":
		case ".tsv":
...
uploadRouter.get("/download-toolkit/:toolkitID", (req, res) => {
	...
	res.setHeader(
		"Content-Disposition",
		`attachment; filename="${toolkitID}_output.zip"`,
	);
...
module.exports = {
	...
	attachHandlers: function (socket, toolkitAnalysisDir) {
		...
		socket.on("getToolkitFiles", (toolkitID) => {
			...
			fs.readdir(dir, (err, files) => {
				...
				if (outputFiles.length > 0) {
					const outputFilePaths = outputFiles.map((file) =>
						path.join(dir, file),
					);
		...
		socket.on("runService", (serviceName, serviceData, formData) => {
			logToFile(
				`[${getCurrentTimestamp()}] Lancement du service : ${serviceName}`,
				socket.id,
			);
	...
	handleOpal(socket, serviceData, uploadedFiles, params, toolkitAnalysisDir) {
		if (!uploadedFiles || !Array.isArray(uploadedFiles)) {
			...
			socket.emit(
				"consoleMessage",
				"Error: no uploaded files or invalid format.",
			);
			return;
		}
		...
		function isSafeValue(value) {
			if (typeof value !== "string" || value.length > 100 || value.length < 1)
				return false;
			const dangerous = [
				";",
				"|",
		...
		function buildOpalLaunchCommand(serviceData, uploadedFiles, params) {
			...
			inputs.forEach((input) => {
				...
				logToFile(
					`Input: ${input.name} type ${input.type} flag ${input.flag}`,
					socket.id,
				);
				if (input.flag) {
				...
				if (input.type === "file" || input.type === "file[]") {
					const matchingFiles = uploadedFiles.filter(
						(file) => file.fieldname === input.name,
					);
			...
			return {
				...
				args: [
					"/opt/OpalPythonClient/opal-py-2.4.1/GenericServiceClient.py",
					...args,
				],
			};
		}
		...
		// Exécution
		const launchInfo = buildOpalLaunchCommand(
			serviceData,
			uploadedFiles,
			params,
		);
		logToFile(
			`Commande: ${launchInfo.binary} ${launchInfo.args.join(" ")}`,
			socket.id,
		);
		socket.emit(
			"consoleMessage",
			`${launchInfo.binary} ${launchInfo.args.join(" ")}`,
		);
				...
				.get(logURL, (res) => {
					...
					res.on("end", () => {
						...
						const outputSection = data
							.split("\n")
							.find((line) => line.includes("Checking expected output files:"));
						...
						if (outputSection) {
							...
							if (fileLines.length > 0) {
								fileLines.forEach((fileName) => {
									...
									downloadFile(outputFileUrl, newFileName, (err) => {
										if (err) {
											logToFile(
												`Erreur download ${fileName}: ${err.message}`,
												socket.id,
											);
											socket.emit(
												"consoleMessage",
											...
											logToFile(
												`Fichier téléchargé: ${newFileName}`,
												socket.id,
											);
											socket.emit(
												"consoleMessage",
											...
											const jobId = toolkitAnalysisDir
												...
												.pop();
											const resultsUrl =
												"https://synflow.southgreen.fr/?id=" + jobId;
											const downloadUrl =
												"https://wsp1453.southgreen.fr/download-toolkit/" +
												jobId;
											const message = `Hello,<br/><br/>Your Synflow workflow has completed successfully.<br/><br/>You can view the results at the following address: <a href="${resultsUrl}">${resultsUrl}</a><br/><br/>Download output files (ZIP): <a href="${downloadUrl}">${downloadUrl}</a><br/><br/><b>Stdout:</b><br/><pre>${stdoutData}</pre><br/><b>Stderr:</b><br/><pre>${stderrData}</pre><br/>Best regards,<br/>The Synflow team`;
							...
							const failedDownloadUrl =
								"https://wsp1453.southgreen.fr/download-toolkit/" + failedJobId;
							const message = `Hello,<br/><br/>Your Synflow workflow has failed. Please check the logs for more information.<br/><br/>Download output files (ZIP): <a href="${failedDownloadUrl}">${failedDownloadUrl}</a><br/><br/><b>Stdout:</b><br/><pre>${stdoutData}</pre><br/><b>Stderr:</b><br/><pre>${stderrData}</pre><br/>Best regards,<br/>The Synflow team`;
							if (email) {
								sendMail(email, "Synflow notification", message);
							}
						} else {
					...
					res.on("end", () => {
						...
						if (data.trim() !== "") {
							...
							const errJobId = toolkitAnalysisDir
								...
								.filter((x) => x)
								.pop();
							const errDownloadUrl =
								"https://wsp1453.southgreen.fr/download-toolkit/" + errJobId;
							const message = `Hello,<br/><br/>Your Synflow workflow has encountered an error. Please check the logs for more information.<br/><br/>Download output files (ZIP): <a href="${errDownloadUrl}">${errDownloadUrl}</a><br/><br/><b>Stdout:</b><br/><pre>${stdoutData}</pre><br/><b>Stderr:</b><br/><pre>${stderrData}</pre><br/>Best regards,<br/>The Synflow team`;
							if (email) {
								sendMail(email, "Synflow notification", message);
							}
						} else {
	...
	handleGalaxy(socket, serviceData, uploadedFiles, params, toolkitAnalysisDir) {
		...
		const launchCommand = buildGalaxyLaunchCommand(
			serviceData,
			uploadedFiles,
			params,
		);
